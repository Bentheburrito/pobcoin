defmodule Buttons do
  ...
  def handle_interaction(%Interaction{data: %{custom_id: "pobcoin_selector:" <> prediction_id_str, values: [pobcoin_amount_str]}} = interaction) do
	...
	def handle_interaction(%Interaction{data: %{custom_id: "close:" <> prediction_id_str}} = interaction) do
    user_id = interaction.member.user.id
		prediction_id = String.to_integer(prediction_id_str)

		case PredictionHandler.close(prediction_id, user_id) do
			:unauthorized ->
				InteractionHandler.respond(interaction, [content: "You didn't create this prediction!"], true)

			{id, prediction} ->
				# update the embed
				outcomes = %{"outcome_1" => prediction["outcome_1"], "outcome_2" => prediction["outcome_2"]}
				embed = SlashCommand.Prediction.create_prediction_embed("[CLOSED] " <> prediction.prompt, outcomes)
				components = SlashCommand.Prediction.create_prediction_components(id, outcomes, true, true)
				Nostrum.Api.edit_interaction_response(Nostrum.Cache.Me.get().id, prediction.token, %{embeds: [embed], components: components})

				# distribute pobcoin

				# send success message
				InteractionHandler.respond(interaction, [content: "Successfully closed interaction!"], true)
		end
  end
