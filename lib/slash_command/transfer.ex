    ...
    cond do
      target_id == interaction.user.id ->
        {:response,
         [
           content:
             "Come on man, that doesn't even make sense. (You can't transfer Pobcoin to yourself)"
         ]}

      amount == 0 ->
        {:response,
         [content: "Pleeeeease stop wasting my time. (You can't transfer zero Pobcoin)"]}
  ...
  defp transfer(%Interaction{} = interaction, %DiscordUser{} = target_user, amount, memo) do
    ...
    case Repo.transaction(multi) do
      {:ok, _map} ->
        ...
        description =
          "Successfully transferred #{amount} Pobcoin to #{target_user}!" <>
        ...
        embed =
          ...
          |> Embed.put_field(
            ...
            "#{receiver.coins} + #{amount} = **#{receiver.coins + amount}**",
            true
          )
      ...
      {:error, :withdraw, %Ecto.Changeset{errors: [coins: {@overdraft_msg, _list}]},
       _changes_so_far} ->
        {:response,
         [
           content:
             "Tbqfh it doesn't seem like you can afford that :/ (Transfer of #{amount} would result in overdraft)"
         ]}

      {:error, fail_op, fail_val, _} ->
        Logger.error(
          "ERROR INSERTING OR UPDATING USER (/transfer #{target_user} #{amount}): #{inspect(fail_op, label: "fail op")} #{inspect(fail_val, label: "fail val")}"
        )
