    ...
    %{
      ...
      options: [
        %{
          ...
          name: "user",
          description:
            "User to give the new Pobcoin to. (seriously if you're not Pob stop using this command maybe ok thx!)",
          required: true
        },
        %{
          ...
          name: "amount",
          description:
            "The amount of Pobcoin to create. (I swear to James's God if you're not Pob I'm gonna-)",
          required: true
        }
      ]
    ...
    cond do
      interaction.member.user.id not in Application.get_env(:pobcoin, :oligarchs, []) ->
        {:response,
         [
           content:
             "You're not pob, impobster (`/give` is for Pobsterlot only - did you mean to use `/transfer`?)"
         ]}

      amount == 0 ->
        {:response,
         [content: "*message deleted by a moderator.*\n\n(You can't create zero Pobcoin)"]}
  ...
  defp give(%Interaction{} = _interaction, target_id, amount) do
    ...
    case Repo.insert_or_update(changeset) do
      {:ok, _map} ->
        ...
        {:response,
         [content: "Successfully created and gave #{amount} Pobcoin to #{target_user}!"]}

      {:error, %Ecto.Changeset{errors: errors}} ->
        Logger.error(
          "ERROR INSERTING OR UPDATING USER (/give #{target_id} #{amount}): #{inspect(errors)}"
        )
